...
export class EtudiantComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.etudiantService
        ...
        .subscribe(
          (res: HttpResponse<IEtudiant[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInEtudiants(): void {
    this.eventSubscriber = this.eventManager.subscribe('etudiantListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IEtudiant[] | null, headers: HttpHeaders, page: number): void {
