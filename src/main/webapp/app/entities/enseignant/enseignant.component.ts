...
export class EnseignantComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.enseignantService
        ...
        .subscribe(
          (res: HttpResponse<IEnseignant[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInEnseignants(): void {
    this.eventSubscriber = this.eventManager.subscribe('enseignantListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IEnseignant[] | null, headers: HttpHeaders, page: number): void {
