...
export class ModuleComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.moduleService
        ...
        .subscribe(
          (res: HttpResponse<IModule[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInModules(): void {
    this.eventSubscriber = this.eventManager.subscribe('moduleListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IModule[] | null, headers: HttpHeaders, page: number): void {
