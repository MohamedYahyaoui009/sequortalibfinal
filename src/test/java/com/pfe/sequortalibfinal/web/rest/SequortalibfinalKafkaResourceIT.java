...
import com.pfe.sequortalibfinal.config.KafkaProperties;
...
class SequortalibfinalKafkaResourceIT {
    ...
    void setup() {
        ...
        SequortalibfinalKafkaResource kafkaResource = new SequortalibfinalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sequortalibfinal-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/sequortalibfinal-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
